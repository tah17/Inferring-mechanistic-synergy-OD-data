...
# Get job idx
#
full_job_idx_list <- expand.grid(Fold = 1:no_of_folds, Chains = 1:no_of_chains, Model = 1:length(model_list))  #prev job list used in 03d_
...
#
# calculate training + pred errors and save
#
fit_list <- lapply(1:no_of_folds, function(x) get_metrics(labelled_data[[x]], stan_fit[[x]]) %>% mutate(fold = x)) 
saveRDS(do.call(rbind, fit_list), paste("output/indv_drug/", model, "_fit_stats.rds", sep = ""))
