...
interaction_drug_matrix <- sapply(combn(2:length(unique(c(fold_df$drug1_name, fold_df$drug2_name))), 2, simplify=FALSE), function(x) single_drug_matrix[, x[1]]*single_drug_matrix[, x[2]])
interaction_drug_matrix <- interaction_drug_matrix[, -(length(unique(c(fold_df$drug1_name, fold_df$drug2_name)))-1)]
...
if (grepl("no_syn", model)) {  # if models are "No Synergy" models:
  stan_data$D_i <- NULL  # drug interactions set to NULL
  stan_data$X <- single_drug_matrix   # only include binary matrix for drugs AFG, MGX and 5FC (not drug interactions)
} 
# Fit Model --------------------------------------------------
