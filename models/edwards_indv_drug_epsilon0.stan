//
// Latent Edwards function modelled with multiplicative noise and individual drug action added but epsilon = 0
//
...
functions {
  vector edwards_drug(real t,
                      ...
                      real max_time) {
    vector[2] dydt;
    dydt[1] = max_time*(beta/(1+gamma))*y[1]*exp(-(y[1]/L)) - kappa_star*y[1]; // ODE for viable fungi scaled by max_time
...
data {
  ...
  int<lower=0, upper=N_train> auris_train_idx[N_auris_train];  // non-blank indices in training
...
transformed parameters {
  ...
  for (i in 1:D_c) {
    ...
    mu_hat[, i, ] = ode_rk45(edwards_drug, to_vector({y0, 0.0}), t_star_train[1], t_star_train[2:T_train], beta, L, X_i*append_row(0.0, kappa_star), X_i*append_row(0.0, gamma), max_train_time);  // solve ODE
    mu[1, i, 1] = y0;
...
generated quantities {
  ...
  for (i in 1:D_c) {  // predictions for CV stratified by rep
    row_vector[D] X_i = to_row_vector(X[i, ]);
    mu_hat_pred[, i, ] = ode_rk45(edwards_drug, to_vector({y0, 0.0}), t_star_test[1], t_star_test[2:T_test], beta, L, X_i*append_row(0.0, kappa_star), X_i*append_row(0.0, gamma), max_train_time);  // solve ODE 
    mu_pred[1, i, 1] = y0;
