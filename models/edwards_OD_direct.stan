//
// Edwards function that assumes OD is direct
//
...
data {
  int<lower=1> N_train;  // number of train obs
  int<lower=0> T_train;  // number of unique train time points
  array[T_train] real train_time;  // unique train time points
  int<lower=1, upper=T_train> time_train_idx[N_train];  // unique train time points indices
  ...
  array[T_test] real test_time;  // unique test time points
  ...
  int<lower=0, upper=N_test> auris_test_idx[N_auris_test];  // indices of non-blanks
...
parameters {
  real<lower=0> y0;  // IC of OD
  real<lower=0> L;  // growth-impedance
...
model {
  ...
  if (include_likelihood) {
    y_obs_train[auris_train_idx] ~ lognormal(log(to_vector(mu[time_train_idx[auris_train_idx], 1])), sigma);
...
generated quantities {
  ...
  y_rep[auris_train_idx] = lognormal_rng(log(to_vector(mu[time_train_idx[auris_train_idx], 1])), sigma);
  ...
  y_pred[auris_test_idx] = lognormal_rng(log(to_vector(mu_pred[time_test_idx[auris_test_idx], 1])), sigma);
  ...
  if (include_likelihood) {
    ...
    for (i in auris_test_idx)
      log_lik[i] = lognormal_lpdf(y_obs_test[i] | log(mu_pred[time_test_idx[i], 1]), sigma);
  }
