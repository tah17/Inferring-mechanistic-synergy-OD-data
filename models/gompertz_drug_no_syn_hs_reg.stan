//
// Latent gompertz function modelled with multiplicative noise with drug action added + regularised HS prior (no synergy)
//
...
data {
  int<lower=0> D;  // number of drugs (with no drug)
  ...
  int<lower=0, upper=N_train> auris_train_idx[N_auris_train];  // auris idxs in training
...
parameters {
  ...
  vector<lower=0>[D-1] epsilon;   // enhancement of reduction of carrying capacity
...
generated quantities {
  ...
  matrix[T_test-1, D_c] f_v_test;  // prediction for viable fungi
