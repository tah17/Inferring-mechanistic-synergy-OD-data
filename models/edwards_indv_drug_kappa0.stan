//
// Latent Edwards function modelled with multiplicative noise and individual drug action added but kappa = 0
//
...
functions {
  vector edwards_drug(real t,
                      ...
                      real max_time) {
    vector[1] dydt;
    dydt[1] = max_time*(beta/(1+gamma))*y[1]*exp(-((y[1]*(1+epsilon))/L)); // ODE of viable fungi scaled by max time
    return dydt;
  }
}
...
parameters {
  ...
  vector<lower=0>[D-1] epsilon;   // enhancement of inherent growth impedance 
}
...
transformed parameters {
  ...
  for (i in 1:D_c) {
    ...
    mu_hat[, i, ] = ode_rk45(edwards_drug, to_vector({y0}), t_star_train[1], t_star_train[2:T_train], beta, L, X_i*append_row(0.0, gamma), X_i*append_row(0.0, epsilon), max_train_time);  // solve scaled Edwards ODE
    mu[1, i, 1] = y0;
    mu[2:T_train, i, 1] = mu_hat[, i, 1];  
  }
}
...
generated quantities {
  ...
  for (i in 1:D_c) {  // predictions for CV stratified by rep
    row_vector[D] X_i = to_row_vector(X[i, ]);
    mu_hat_pred[, i, ] = ode_rk45(edwards_drug, to_vector({y0}), t_star_test[1], t_star_test[2:T_test], beta, L, X_i*append_row(0.0, gamma), X_i*append_row(0.0, epsilon), max_train_time);  // solve scaled ODE
    mu_pred[1, i, 1] = y0;
    mu_pred[2:T_test, i, 1] = mu_hat_pred[, i, 1];
  }
