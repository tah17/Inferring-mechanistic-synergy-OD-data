//
// Latent exponential function modelled with multiplicative noise 
//
data {
  ...
  int<lower=0, upper=N_train> auris_train_idx[N_auris_train];   // idxs of non blanks in training
  ...
  array[T_test] real test_time;  // prediction times
  int<lower=1, upper=T_test> time_test_idx[N_test];   // idxs of pred times
  ...
  int<lower=0, upper=N_test> auris_test_idx[N_auris_test];   // idxs of non blanks 
...
parameters {
  ...
  real<lower=0> y0;  // initial value 
}

transformed parameters {
  vector[N_auris_train] f;
  real<lower=IC> delta = pow(10, delta_tilde);  // linear transform parameter (scale)
  for (i in 1:N_auris_train) f[i] = y0*exp(beta*train_time[time_train_idx[auris_train_idx[i]]]);  // exponential function
}
...
generated quantities {
  ...
  for (i in 1:N_auris_test) f_pred[i] = y0*exp(beta*test_time[time_test_idx[auris_test_idx[i]]]); // exponential function
