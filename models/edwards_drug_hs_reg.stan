//
// Latent Edwards function modelled with multiplicative noise and added drug action (with synergy) + regularised HS prior
//
...
data {
  ...
  int<lower=1, upper=T_test> time_test_idx[N_test];  // test time points idxs
...
transformed parameters {
  ...
  for (i in 1:D_c) {
    ...
    mu_hat[, i, ] = ode_rk45(edwards_drug, to_vector({y0, 0.0}), t_star_train[1], t_star_train[2:T_train], beta, L, X_i*append_row(0.0, kappa_star), X_i*append_row(0.0, gamma), X_i*append_row(0.0, epsilon), max_train_time); // solve ODE
    mu[1, i, 1] = y0;
