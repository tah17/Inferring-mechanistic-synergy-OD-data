//
// Latent Edwards function modelled with multiplicative noise and individual drug action added + regularised HS prior
//
...
data {
  ...
  real max_train_time;  //  max time in training
...
generated quantities {
  ...
  for (i in 1:D_c) {  // predictions for CV stratified by rep
    row_vector[D] X_i = to_row_vector(X[i, ]);
    mu_hat_pred[, i, ] = ode_rk45(edwards_drug, to_vector({y0, 0.0}), t_star_test[1], t_star_test[2:T_test], beta, L, X_i*append_row(0.0, kappa_star), X_i*append_row(0.0, gamma), X_i*append_row(0.0, epsilon), max_train_time);  // solve Edwards ODE
    mu_pred[1, i, 1] = y0;
