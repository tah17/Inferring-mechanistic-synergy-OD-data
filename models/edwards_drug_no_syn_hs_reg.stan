//
// Latent Edwards function modelled with multiplicative noise and drug action added + regularised HS prior (no synergy)
//
...
data {
  ...
  int<lower=0, upper=N_train> auris_train_idx[N_auris_train];  // non-blanks indices
...
parameters {
  ...
  real<lower=0> caux;
  vector<lower=0>[(D-1)*3] lambda;  // local regularisation for each drug
  real<lower=0> tau;  // global regularisation
}

transformed parameters {
  real<lower=0> c = slab_scale*sqrt(caux);
  vector<lower=0>[(D-1)*3] lambda_tilde = sqrt(c^2 * square(lambda) ./ (c^2 + tau^2 * square(lambda)));
  ...
  for (i in 1:D_c) {
    ...
    mu_hat[, i, ] = ode_rk45(edwards_drug, to_vector({y0, 0.0}), t_star_train[1], t_star_train[2:T_train], beta, L, X_i*append_row(0.0, kappa_star), X_i*append_row(0.0, gamma), X_i*append_row(0.0, epsilon), max_train_time);  // solve Edwards ODE
    mu[1, i, 1] = y0;
...
model {
  ...
  for (i in 1:(D-1)) {
    ...
    gamma[i] ~ normal(0, lambda_tilde[i+(D-1)]*tau);
    epsilon[i] ~ normal(0, lambda_tilde[i+(2*(D-1))]*tau);
  }  
...
generated quantities {
  vector<lower=0>[D-1] kappa = kappa_star/max_train_time; // killing rate in hrs^(-1)
  ...
  for (i in 1:D_c) {  // predictions for CV stratified by rep
    row_vector[D] X_i = to_row_vector(X[i, ]);
    mu_hat_pred[, i, ] = ode_rk45(edwards_drug, to_vector({y0, 0.0}), t_star_test[1], t_star_test[2:T_test], beta, L, X_i*append_row(0.0, kappa_star), X_i*append_row(0.0, gamma), X_i*append_row(0.0, epsilon), max_train_time);  // solve ODE
    mu_pred[1, i, 1] = y0;
