//
// Latent logistic function modelled with multiplicative noise 
//
data {
  ...
  int<lower=0, upper=N_train> auris_train_idx[N_auris_train];  // non blank idxs
  ...
  int<lower=0, upper=N_test> auris_test_idx[N_auris_test];   // non blank idxs
...
transformed parameters {
  ...
  for (i in 1:N_auris_train) f[i] = K/(1 + ((K - y0)/y0)*exp(-(beta)*train_time[time_train_idx[auris_train_idx[i]]]));  // logistic function
}
...
generated quantities {
  ...
  for (i in 1:N_auris_test) f_pred[i] = K/(1 + ((K - y0)/y0)*exp(-(beta)*test_time[time_test_idx[auris_test_idx[i]]]));  // logistic function
