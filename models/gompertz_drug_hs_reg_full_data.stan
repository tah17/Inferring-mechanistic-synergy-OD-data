//
// Latent Gompertz function modelled with multiplicative noise with drug action added + regularised HS prior fit to all data
//
...
data {
  ...
  int<lower=0> N_auris;  // number of data points for auris 
  int<lower=0, upper=N> auris_idx[N_auris];  // auris idxs
...
transformed parameters {
  ...
  array[T-1, D_c] vector[1] mu_hat;
  array[T, D_c] vector[1] mu;
  for (i in 1:D_c) {
    row_vector[D+D_i] X_i = to_row_vector(X[i, ]);
    real beta_gamma_max_time = (max_time*beta)/(1+X_i*append_row(0.0, gamma));
    real K_epsilon = K/(1+X_i*append_row(0.0, epsilon));
    real kappa_tilde = X_i*append_row(0.0, kappa_star);
    f_v[, i] = y0*exp((log(K_epsilon/y0)-(kappa_tilde/beta_gamma_max_time))*(1-exp(-beta_gamma_max_time*to_vector(t_star[2:T]))));  // gompertz scaled by max time for viable fungi
    mu_hat[, i, ] = ode_rk45(gompertz_drug, to_vector({0.0}), t_star[1], t_star[2:T], y0, beta_gamma_max_time, K_epsilon, kappa_tilde);  // solve ODE for dead fungi
    mu[1, i, 1] = 0;
    f_v[1, i] = y0;
    mu[2:T, i, 1] = to_array_1d(to_vector(f_v[, i]) + to_vector(mu_hat[, i, 1]));  // viable and dead fungi added
  }
}
